...
while indexDeltaY <= maxIndexDeltaY
    ...
    clc       
    [indexK indexA indexDeltaY Karray(1) aArray(1) deltaYarray(1)*1000]
    [maxIndexK maxIndexA maxIndexDeltaY Karray(end) aArray(end) deltaYarray(end)*1000]
...
% K loop
    ...
    %if mod(indexK,5) == 1
        ...
        title(['Fitting error at K = ' num2str(Karray(indexK-1)) ', optimal K = ' num2str(Ksave) ]);
        pause(0.1);
    %end   
